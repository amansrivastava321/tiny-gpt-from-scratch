...
def softmax_overflow_demo(large_value):
    ...
    overflowed = np.any(np.isinf(naive_exp))
    return {'naive_exp': float(np.max(naive_exp)), 'overflowed': bool(overflowed)}
...
def sample_random_batch_offsets(data_len, block_size, batch_size, rng):
    ...
    window_needed = data_len - block_size 
    offsets = rng.integers(0,window_needed, size=batch_size )
    return offsets
...
def stack_x_batch(data, offsets, block_size):
    ...
    for offset in offsets:
        window = data[offset:offset+block_size]
...
def stack_y_batch(data, offsets, block_size):
    ...
    y_windows =[]
    for offset in offsets:
        y_window = slice_y_at_offset(data, offset, block_size)
        y_windows.append(y_window)
    return np.stack(y_windows)
...
def allocate_count_matrix(vocab_size):
    ...
    N = make_2d_zeros(vocab_size, vocab_size)
...
def generate_sequence(p_matrix, start_id, length, rng):
    ...
    for _ in range(length-1):
        next_id = sample_next_token(p_matrix, curr_id, rng)
        seq.append(next_id)
        curr_id = next_id
...
def log_prob_of_pair(p_matrix, current_id, next_id):
    ...
    N = index_element(p_matrix, current_id, next_id)
    return array_log(N)
...
def sum_negative_log_probs(p_matrix, data):
    ...
    for t in range(len(data)-1):
        current_id = data[t]
        next_id = data[t+1]
        N = log_prob_of_pair(p_matrix, current_id, next_id)
        total += -N
    return total
...
def average_nll(p_matrix, data):
    ...
    N = sum_negative_log_probs(p_matrix, data)
